 ...
 *
 * IMPLEMENTADO: verificação, validação de assinatura HMAC, registro em
 * webhook_events com deduplicação, resposta 200 imediata (evita retry da Meta).
 * PENDENTE DE CREDENCIAL: nenhuma chamada de saída (envio de mensagem) é feita
 * ainda por esta rota — isso é Sprint 2/7, feito depois que a análise de IA e
 * o roteamento de conversa estiverem prontos.
 */
...
export async function POST(request: NextRequest) {
  ...
  // O processamento pesado (classificação IA, criação de contato/oportunidade)
  // acontece a partir do registro em webhook_events — ver PENDENTE DE
  // CONFIGURAÇÃO abaixo.
  return NextResponse.json({ received: true }, { status: 200 });
}
...
async function recordAndMaybeProcessEvent(params: {
  ...
  // PENDENTE: disparar aqui o processamento assíncrono (identificar contato,
  // criar/atualizar conversa, classificar mensagem, etc.) — Sprint 3/7.
  // Por ora o evento fica registrado como processed=false para ser
  // processado por um job subsequente, sem risco de perda de mensagem.
}
